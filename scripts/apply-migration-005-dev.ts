  ...
  supabase: {
    from: (table: string) => {
      select: (columns: string) => {
        limit: (count: number) => PromiseLike<{ error: { code?: string } | null }>;
      };
    };
  },
): Promise<boolean> {
...
async function applyMigrationWithPostgres(
  ...
  const sqlPath = resolve(
    projectRoot,
    "supabase/migrations/005_video_snapshots_subscriber_count.sql",
  );
...
async function main(): Promise<void> {
  ...
  if (await columnExists(supabase)) {
    console.log("subscriber_count column already exists on video_snapshots.");
    return;
  }

  if (!env.SUPABASE_DB_PASSWORD) {
    console.error("subscriber_count column is missing on dev DB.");
    console.error(
      "Add SUPABASE_DB_PASSWORD to .env.local, then rerun: npm run db:apply:005-dev",
    );
    console.error(
      "Or paste supabase/migrations/005_video_snapshots_subscriber_count.sql into Supabase SQL Editor.",
    );
    process.exit(1);
  }

  console.log("Applying 005_video_snapshots_subscriber_count.sql to dev DB...");
  await applyMigrationWithPostgres(env);

  if (!(await columnExists(supabase))) {
    throw new Error("Migration applied but subscriber_count column is still missing");
  }
